...
* protocoles de transport de données
  * [TCP](#tcp)
  * [UDP](#udp)
* adressage IP
  * LAN/WAN
  * plages d'IP
* concept et utilisation du NAT

# Protocole de transport de données

Les protocole de transport de données correspondent à la couche 4 du modèle OSI. Ca veut dire que : 
...
* communication qui ne nécessite pas 100% d'intégrité
  * jeu en ligne
...
Inutile de s les apprendre par coeur, c'est à titre informatif, voici les principales plages d'adresses IP réservée :
...
**Le NAT (*Network Address Translation*) est une fonction que peut porter un routeur.** Cela permet à des machines avec adresses privées de communiquer avec des machines possédant des IP publiques (passer d'un domaine privé/local à un domaine publique/global).
